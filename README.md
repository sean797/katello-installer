...
1. When running katello-installer for the first time:
...
2. Setting custom certificates after katello-installer was already
run.

The first run `katello-installer` uses the default CA for both server
...
enforce deploying new certificates for the custom server CA, one runs
...
For updating the certificates on a capsule, one passes the same
